...
# INPUTS:
# df = census dataframe
### At the state level, it is "df_state.rda"
### At the district level, it is "df_district.rda"

# poll = poll dataframe

# key = geography-level specific crosswalk connecting the poll, census, and shapefiles
### At the state level, it is "xwalk_state.xlsx"
### At the district level, it is "xwalk_district.xlsx"

# col_vars = vector of demographic variables included in the df that you want to output breakdowns by. 

# column_name = name of the column in the df with the geographic level 
### At the state level, it is "state_code"
### At the district level, it is "state_dist_code"

# model = model equation

# target_time = current year

# xsim = Binary true/false indicating whether or not to run bootstrapping simulations to calculate confidence intervals. Defaul is TRUE. 

# n_sims = Number of bootstrapping simulations to run if "xsim" is true. Default is 99. 

# OUTPUTS:
# Inputs for the poststrat_save function
...
#==============================================================================#
todaysdate <- Sys.Date()
logfile <- paste0(outputpath,"logs/poststrat_functions_",todaysdate,"_log.txt")
sink(logfile, append=TRUE, split=TRUE)
#==============================================================================#
## POSTSTRAT FUNCTION ####
#==============================================================================#
poststrat <- function(df, poll, key, column_name, model, target_time, col_vars, xsim=TRUE, n_sims=99){ 
