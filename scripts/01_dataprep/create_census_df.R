...
df_district <- df_district %>%
  dplyr::mutate(GEOID = ifelse(GEOID %in% c("25494", "25495"), "25495", GEOID),
                district_code = ifelse(district_code %in% c("494", "495"), "495", district_code),
                state_dist_code = ifelse(state_dist_code %in% c("25494", "25495"), "25495", state_dist_code)) %>%
  dplyr::group_by(GEOID, state_dist_code, state_code, district_code, urban, caste, gender, age) %>%
  dplyr::summarise(N = sum(N),
                   n = mean(n),
                   n_pct_geo = mean(n_pct_geo)) %>%
  dplyr::ungroup()
