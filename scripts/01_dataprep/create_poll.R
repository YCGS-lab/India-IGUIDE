...
xwalk_path <- paste0(dropbox,"xwalk/")
...
# Create District Codes for Survey Data
districts <- poll %>%
  dplyr::select(district) %>%
  dplyr::distinct() 

districts <- districts %>%
  dplyr::mutate(district_survey_code = seq(1, nrow(districts), 1))

poll <- base::merge(poll, districts, by="district", all=TRUE)
...
poll <- poll %>%
  dplyr::select(caseid, weight, wave, state, district, state_district, district_survey_code,
                age, gender, caste, urban, ends_with("_recode"))

# Calculate the number of states and districts included in the survey
xwalk <- xwalk %>%
  dplyr::select(-contains("shape23")) %>%
  dplyr::distinct()

poll_temp <- base::merge(poll, xwalk, 
                         by.x=c("state", "district", "state_district"),
                         by.y=c("state_survey", "district_survey", "state_district_survey"),
                         all.x = TRUE)
length(unique(poll_temp$state_census11))
length(unique(poll_temp$state_district_census11))
