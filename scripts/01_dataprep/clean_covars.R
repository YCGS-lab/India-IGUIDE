...
higher_ed <- cd %>% 
  dplyr::mutate(district.code = ifelse(district.code %in% c("25494", "25495"), "25495", district.code)) %>%
...
# Average Daman & Diu
temp <- covars %>%
  dplyr::filter(state_census11 == "Daman & Diu") %>%
  dplyr::group_by(district_census11_code, state_district_census11_code, zone, 
                  state_census11, state_census11_code, district_census11, 
                  state_district_census11, country) %>%
  dplyr::summarise(across(where(is.numeric), \(x) mean(x, na.rm = TRUE)))  %>%
  dplyr::ungroup()
temp <- base::merge(temp, xwalk, 
                    by=c("district_census11_code", "state_district_census11_code", "zone", 
                         "state_census11", "state_census11_code", "district_census11", 
                         "state_district_census11", "country"), 
                    all.x=TRUE)
temp <- temp[, names(covars)]
covars <- covars[covars$state_census11 != "Daman & Diu",]
