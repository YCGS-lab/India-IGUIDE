...
PROJECT_ROOT <- ("~/YSE Dropbox/Emily Goddard/India-IGUIDE/")
RAW_DATA_PATH <- file.path(PROJECT_ROOT, "_data/poll/poll_covariates.rds")
SHAPEFILE_PATH <- file.path(PROJECT_ROOT,
                            "_data/shapefile/district_shapefile.shp")
XWALK_PATH <- file.path(PROJECT_ROOT,
                        "_data/xwalk/xwalk_full.xlsx")
RESULTS_DIR <- file.path(PROJECT_ROOT, "output_flood/")
...
#Crosswalk
xwalk <- read.xlsx(XWALK_PATH)

TARGET <- "n7fy23_flood_worry_recode"
...
data <- base::merge(data, xwalk, by=c("state_survey", "district_survey", "state_district_survey"),
                    all.x=TRUE) %>%
  dplyr::distinct()

data$district_census11_code <- as.integer(data$district_census11_code)
data$district_shape23_code <- as.integer(data$district_shape23_code)
...
split_map <- shp_utm %>%
  st_drop_geometry() %>%
  dplyr::select(all_of(shp_code_col), split)

data <- left_join(data, split_map,
                  by=setNames(shp_code_col,"district_shape23_code"))
...
# ============================================================
district_data <- data %>%
  dplyr::group_by(district_shape23_code) %>%
  dplyr::summarise(worry_rate=mean(.data[[TARGET]], na.rm=TRUE))
...
spatial_cols <- c("dist_cd", SPATIAL_LAG_COLS, LISA_COLS)

spatial_df <- geo_districts %>%
  st_drop_geometry() %>%  
  dplyr::select(all_of(spatial_cols))

# Merge with main data
data <- data %>%
  dplyr::select(-all_of(SPATIAL_LAG_COLS), -all_of(LISA_COLS))

data <- base::merge(data, spatial_df, by.x = "district_shape23_code", by.y= "dist_cd")
...
# M1: Demographics only
M1 <- c("age","gender","urban","caste","district_shape23_code")
...
data <- data %>%
  dplyr::mutate(across(all_of(categorical_cols), ~as.integer(as.factor(.x))))

# ============================================================
#  TAKE AVERAGE OF VALUES BY CASEID
# ============================================================
# Remove crosswalk
data <- data %>%
  dplyr::select(-contains("shape23"), -contains("_survey")) %>%
  dplyr::mutate(state_district_census11_code = as.numeric(state_district_census11_code)) %>%
  dplyr::group_by(caseid, state_census11, state_census11_code, 
                  district_census11, district_census11_code, 
                  state_district_census11, state_district_census11_code, wave,
                  age, gender, caste, urban, zone, country) %>%
  dplyr::summarise(split = first(split),
                   across(where(is.numeric), mean, na.rm = TRUE)) %>% 
  dplyr::ungroup() %>%
  dplyr::distinct() 

data$caseid[duplicated(data$caseid)]
...
train_d <- length(unique(data$state_district_census11[data$split=="train"]))
test_d <- length(unique(data$state_district_census11[data$split=="test"]))

cat("Total data:", length(data$caseid),  "\nTraining data:", train_n, "\nTesting data:", test_n)

cat("Total districts:", length(unique(data$state_district_census11)),  "\nTraining data:", train_d, "\nTesting data:", test_d)

# Write Poll Data formatted for Modeling
saveRDS(data, paste0(PROJECT_ROOT,"_data/poll/poll_model.rds"))
write.csv(data, paste0(PROJECT_ROOT,"_data/poll/poll_model.csv"))
# ============================================================
...
for(scenario_name in names(SCENARIOS)) {
  ...
  features[features=="district_shape23_code"] <- "state_district_census11_code"
  cat("\n--------------------------------------------------\n")
  ...
  X_train <- train_data %>%
    dplyr::select(all_of(features)) %>% mutate_all(~ifelse(is.na(.), 0, .))
  X_test  <- test_data %>%
    dplyr::select(all_of(features)) %>% mutate_all(~ifelse(is.na(.), 0, .))
  y_train <- train_data %>% dplyr::select(all_of(TARGET))
  y_test  <- test_data %>% dplyr::select(all_of(TARGET))
...
best_per_scenario <- results_df %>%
  group_by(Scenario) %>%
  slice_max(AUC, n = 1) %>%
  dplyr::select(Scenario, Classifier, AUC)
...
model_full_path <- file.path(RESULTS_DIR, paste0("model/",model_name))
...
write.csv(results_df,
          file.path(RESULTS_DIR,"model/model_comparison_results.csv"),
          row.names=FALSE)
