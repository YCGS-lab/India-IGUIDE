...
column_name <- "state_district_census11_code"
...
# Model file
model <- readRDS(paste0(outdir,"model/best_model.rds"))
...
statepred <-  areapred %>%
  ...
  dplyr::select(state_census11, state_per)

areapred <- base::merge(areapred, statepred, by="state_census11", all.x=TRUE)

# Label names and codes for all geographies
areapred_mapping <- areapred %>% 
...
areapred_write <- areapred %>%
  ...
  dplyr::select(GeoName, GeoID, state, zone, n, cellpred_n, pred_prop, pred_per, national_per, state_per, pred_per_diff)
...
write.csv(areapred, file=paste0(outdir,"final/flood_district_validation.csv"), row.names = FALSE)
saveRDS(areapred, file=paste0(outdir,"final/flood_district_validation.rds"))

# Save files for mapping
write.csv(areapred_mapping, file=paste0(outdir,"final/flood_district_mapping.csv"), row.names = FALSE)
saveRDS(areapred_mapping, file=paste0(outdir,"final/flood_district_mapping.rds"))
...
### 5. Save state files ####
write.csv(statepred, file=paste0(outdir,"final/flood_state_table.csv"), row.names = FALSE)
saveRDS(statepred, file=paste0(outdir,"final/flood_state_table.rds"))

### 6. Save country files ####
