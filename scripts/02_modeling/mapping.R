...
natpred <- as.data.frame(readRDS(paste0(outdir,"final/flood_country_table.rds")))

# Load the shapefile
shape_district <- read_sf(dsn=paste0(input, "shapefile/"), layer=paste0("district_shapefile"))
...
df <- df %>%
  ...
  dplyr::distinct() 
# %>%
#   dplyr::filter(!is.na(pred_per))

# Identify national average
nat_avg <- round(as.numeric(natpred$pred_per),0)
